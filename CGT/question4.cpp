...
vector<Edge> findMSTPrim(vector<vector<int>>& adjMatrix, int n) {
    vector<Edge> mst;
...
// DFS function to find a path from u to v in the MST
    ...
    for (const auto& edge : allEdges) {
        ...
        if (!inMST) {
            ...
            if (dfsFindPath(edge.u, edge.v, adjMST, path, visited)) {
                path.push_back(edge.u); // Add starting vertex to close the circuit
